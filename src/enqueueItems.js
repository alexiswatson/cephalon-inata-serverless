...
export const lambda = ({ market, sqs }) => async function enqueueItems(event) {
	...
	const batchEntries = (entries) => {
		...
		return (!batch.length)
			? entries
			: [batch].concat(batchEntries(entries.slice(10, entries.length)));
	};
	...
	const batchedResponses = await Promise.all(
		batchedEntries.map((Entries) => sqs.sendMessageBatch({ Entries, QueueUrl }).promise()),
	);

	const failedResponses = batchedResponses.reduce(
		(failedSoFar, { Failed }) => ((Failed) ? [...failedSoFar, ...Failed] : failedSoFar),
		[],
	);
...
export const handler = lambda({ market: marketInstance, sqs: sqsInstance });
