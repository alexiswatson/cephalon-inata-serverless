...
import InfluxDBAPIFactory from './lib/InfluxDBAPI';

const ssm = new AWS.SSM();
const paramStoreInstance = SecureParameterStore.create(ssm);
const marketInstance = WarframeMarketAPI.getInstance();
...
export const lambda = (deps) => async function retrieveItemStats(event) {
	const { market, paramStore, InfluxDBAPI } = deps;
	const { Records } = event;
	...
	const itemPoints = itemStats.map((item) => ({
		...item,
		points: item.stats.statistics_closed['48hours'].map((point) => new Point('closed_trades_hourly')
...
export const handler = lambda({
	...
	paramStore: paramStoreInstance,
	InfluxDBAPI: InfluxDBAPIFactory,
});
