 ...
 */
export default function sendMessageBatchStubImpl(items, succeedsIf) {
	...
	return items.reduce(({ Successful, Failed }, item) => (succeedsIf(item)
		? { Successful: [...Successful, success(item)], Failed }
		: { Successful, Failed: [...Failed, fail(item)] }), { Successful: [], Failed: [] });
}
